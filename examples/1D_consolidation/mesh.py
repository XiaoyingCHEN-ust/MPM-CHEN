...
# mpm 2d mesh
nodes,elem = mpm_mesh2d(xmin=0, xmax=0.02, ymin=0, ymax=1.1, sizex=0.02, sizey=0.02)
save_mesh(nodes, elem, "mesh")

# gimp 2d mesh
# nodes,elem = gimp_mesh2d(xmin=-0.5, xmax=1.5, ymin=0.0, ymax=2.5, sizex=0.1, sizey=0.1)
# save_mesh(nodes, elem, "gimp_mesh2d")

# find boundary node set
boundary = numpy.where(nodes[:,1]==0)
save_entries(boundary, "bottom_node_id")

# find boundary node set
boundary = numpy.where(nodes[:,1]==1.0)
save_entries(boundary, "top_node_id")

# find boundary node set
boundary = numpy.where(nodes[:,0]==0)
save_entries(boundary, "left_node_id")

# find boundary node set
boundary = numpy.where(nodes[:,0]==0.02)
save_entries(boundary, "right_node_id")

# create and save particle
particles = cube_particle2d(xmin=0, xmax=0.02, ymin=0, ymax=1.0, size=0.01)
save_particle(particles, "particles")

# find boundary particles set
boundary = numpy.where(particles[:,1]>=(1.0-0.02))
save_entries(boundary, "top_particle_id")
...
porepressure[:, 1][particles[:, 0] < 3] = -10000
porepressure[:, 1][particles[:, 0] >= 3] = -10000
...
ax.plot(particles[:,0],particles[:,1],'.')
ax.axis('equal')
plt.show()
